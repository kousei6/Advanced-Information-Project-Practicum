ご要望のデータ（タイミング、座標、圧力、指の面積など）を取得し、**CSV形式でダウンロードできる機能**を追加しました。

データを正確に取得するために、単純なクリックイベントではなく、**Pointer Events API**（`pointerdown`, `pointerup`）を使用するように変更しています。これにより、キーを押している時間や圧力などが計測可能になります。

### 追加・変更された機能

1.  **データ記録機能**: キー操作ごとに以下のデータをメモリに保存します。
      * **Timestamp**: 打鍵時刻
      * **Key**: 押されたキー
      * **Hold Time**: キーを押していた時間（ms）
      * **Down-Down Time**: 前のキーを押してから今回のキーを押すまでの時間（ms）
      * **Up-Down Time**: 前のキーを離してから今回のキーを押すまでの時間（ms）
      * **Keyboard Scale**: その瞬間のキーボードの拡大率
      * **Keyboard X/Y**: その瞬間のキーボードの画面上の絶対座標
      * **Pressure**: 押下圧力（対応デバイスのみ。マウスは0.5固定など）
      * **Finger Area**: 接地面積（幅x高さで概算。対応デバイスのみ）
2.  **CSVダウンロードボタン**: 画面下部にボタンを追加し、記録したデータを即座にCSVファイルとして出力します。
